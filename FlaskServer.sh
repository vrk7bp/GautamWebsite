wget http://pypi.python.org/packages/source/W/Werkzeug/Werkzeug-0.8.3.tar.gz
tar -xvzf Werkzeug-0.8.3.tar.gz
cp -r Werkzeug-0.8.3/werkzeug ~/GautamWebsite/
wget http://pypi.python.org/packages/source/J/Jinja2/Jinja2-2.6.tar.gz
tar -xvzf Jinja2-2.6.tar.gz
cp -r Jinja2-2.6/jinja2 ~/GautamWebsite/
wget http://pypi.python.org/packages/source/s/simplejson/simplejson-3.0.7.tar.gz
tar -xvzf simplejson-3.0.7.tar.gz
cp -r simplejson-3.0.7/simplejson ~/GautamWebsite/